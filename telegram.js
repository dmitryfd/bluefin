...
bot.command('rates', async (ctx) => {
    try {
        ...
        for (let i = 0; i < Math.min(fixedMortgages.length, 5); ++i) {
            msg += `- [${fixedMortgages[i].provider}](${fixedMortgages[i].url}): ${fixedMortgages[i].value}%\n`;
        }

        msg += '\n*Variable rates:*\n';
        for (let i = 0; i < Math.min(variableMortgages.length, 5); ++i) {
            msg += `- [${variableMortgages[i].provider}](${variableMortgages[i].url}): P${variableMortgages[i].valueP.toPrecision(2)}% (${variableMortgages[i].value}%)\n`;
        }
